...
class sapNetweaverProviderInstance(ProviderInstance):
    ...
    def _validateRfcClient(self) -> None:
        ...
        sapHostnameStr = "%s|%s" % (dispatcherInstance['hostname'], dispatcherInstance['instanceNr'])

        self.tracer.info("%s attempting to fetch SMON metrics from %s", self.logTag, sapHostnameStr)
        ...
        self.tracer.info("%s successfully queried SMON metrics from %s", self.logTag, sapHostnameStr)

        self.tracer.info("%s attempting to fetch SWNC workload metrics from %s", self.logTag, sapHostnameStr)
        result = client.getSwncWorkloadMetrics(startDateTime=startTime, endDateTime=endTime)
        self.tracer.info("%s successfully queried SWNC workload metrics from %s", self.logTag, sapHostnameStr)

        self.tracer.info("%s successfully validated all known RFC SDK calls", self.logTag)
    ...
    def getInstances(self, 
        ...
        if ('hostConfig' in self.state and useCache):
            self.tracer.info("%s using cached list of system instances", self.logTag)
...
class sapNetweaverProviderCheck(ProviderCheck):
    ...
    def _actionGetSmonAnalysisMetrics(self) -> None:
        ...
        try:
            ...
            # RFC connections against direct application server instances can only be made to 'ABAP' instances
            dispatcherInstance = self.providerInstance.getActiveDispatcherInstance()
            ...
            self.tracer.info("%s successfully queried SMON metrics for %s", self.logTag, sapHostnameStr)
    ...
    def _actionGetSwncWorkloadMetrics(self) -> None:
        ...
        try:
            ...
            self.tracer.info("%s successfully queried SWNC workload metrics for %s", self.logTag, sapHostnameStr)
            self.lastRunLocal = datetime.now(timezone.utc)
            self.lastRunServer = endTime
